...
func (buffer *LinkSendBuffer) isBlocked() bool {
	...
	if blocked {
		pfxlog.ContextLogger(buffer.x.Label()).Infof("blocked=%v win_size=%v tx_buffer_size=%v rx_buffer_size=%v", blocked, buffer.windowsSize, buffer.linkRecvBufferSize, buffer.linkSendBufferSize)
	}
